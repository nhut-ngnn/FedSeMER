...
def train_and_evaluate(model, train_dataset, valid_dataset, test_dataset,
    ...
    init_kwargs = {
        "project": log_context.get("project", "FedSeMER-EmotionRecognition-{dataset_name}"),
    ...
    try:
        ...
        if os.path.exists(save_path):
            try:
                ...
                print(f"[WARN] Failed to load checkpoint {save_path}: {exc}. Using current model state.")
