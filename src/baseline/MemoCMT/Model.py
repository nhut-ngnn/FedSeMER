...
class MemoCMT(nn.Module):
    def __init__(self, cfg: Config, device: str = "cpu"):
        ...
        # Project both modalities to a shared attention dimension first. Text
        # features may include appended NRC features (e.g. 768 + 10 = 778),
        # which is not necessarily divisible by the number of attention heads.
    ...
    def forward(self, input_text: torch.Tensor, input_audio: torch.Tensor, return_all: bool = False, output_attentions: bool = False):
        ...
        text_projected = self.text_layer_norm(self.text_linear(text_embeddings))
        audio_projected = self.audio_layer_norm(self.audio_linear(audio_embeddings))

        # Text cross attention: Q=audio, K=V=text (in the shared space).
        text_attention, text_attn_output_weights = self.text_attention(
            audio_projected, text_projected, text_projected, average_attn_weights=False
        )
        text_norm = self.dropout(text_attention)

        # Audio cross attention: Q=text, K=V=audio (in the shared space).
        audio_attention, audio_attn_output_weights = self.audio_attention(
            text_projected, audio_projected, audio_projected, average_attn_weights=False
        )
        audio_norm = self.dropout(audio_attention)
