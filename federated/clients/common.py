...
def build_model(num_classes, model_name="fedsemer", cfg=None):
...
@torch.no_grad()
def compute_pseudo_label_stats(model, unlabeled_dataset, threshold, batch_size, modality="both"):
    """Measure accepted pseudo-label count/mass using the incoming global model."""
    if unlabeled_dataset is None or len(unlabeled_dataset) == 0:
        return {"accepted_count": 0, "confidence_mass": 0.0}
    ...
    for batch in loader:
        ...
        confidence_mass += float(confidence[accepted].sum().item())
...
def evaluate_global(client_id, cfg, features_dir, state_dict=None, split="test", state_path=None):
    cfg = dict(cfg)
    labeled_samples, labeled_path = _load_split(features_dir, "train_labeled")
    ...
    model = build_model(
        cfg["num_classes"],
        cfg.get("model_name", "fedsemer"),
        cfg=cfg,
    ).to(train_device)
    if state_dict is None and state_path:
        state_dict = torch.load(state_path, map_location="cpu")
    if state_dict is None:
        raise ValueError("evaluate_global requires state_dict or state_path.")
