...
def resolve_worker_count(num_workers, num_tasks, operation="federated operation"):
    requested = max(1, int(num_workers or 1))
    workers = min(requested, max(1, int(num_tasks)))
    if workers > 1 and torch.cuda.is_available():
        print(
            f"[WARN] CUDA is available; running {operation} sequentially "
            f"(requested num_workers={requested}) to avoid multiple CUDA contexts.",
            flush=True,
        )
        return 1
    return workers
