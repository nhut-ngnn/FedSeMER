...
def _format_seed_list(value):
    if value is None:
        return None
    if isinstance(value, (list, tuple)):
        seeds = [str(item).strip() for item in value if str(item).strip()]
...
def _load_yaml_config(path):
    ...
    defaults["val_ratio"] = loso.get("val_ratio")
    ...
    defaults["seeds"] = train.get("seeds")
...
def _ensure_centralized_feature_names(client_feat_dir, dataset, num_classes):
    ...
    for split, expected_name in expected.items():
        ...
        if os.path.lexists(expected_path) and not os.path.isfile(expected_path):
...
def parse_args():
    ...
    parser.add_argument("--val_ratio", type=float, default=0.2,
                        help="Validation ratio sampled from the non-test train pool when val_session is not set.")
    ...
    parser.add_argument("--seeds", type=str, default=None,
                        help="Comma-separated seed list for centralized/train.py.")
    ...
    args.seeds = _format_seed_list(args.seeds)
...
def main():
    ...
    for fold in fold_items:
        ...
        train_sessions = []
        val_from_train_ratio = args.val_session is None
        if args.val_session is not None:
            ...
            val_session = None
            val_pct = int(round(float(args.val_ratio) * 100))
            val_tag = "clientval" if args.local_only else "trainval"
            fold_name = f"test_session{test_session}_{val_tag}{val_pct}"

        if args.model_name == "fedsemer":
        ...
        excluded_sessions = {test_session}
        if val_session is not None:
            excluded_sessions.add(val_session)
        train_sessions = [sess for sess in sessions if sess not in excluded_sessions]
        train_pool = []
        for sess in train_sessions:
            train_pool.extend(session_map[sess])
        test_samples = list(session_map[test_session])

        rng = random.Random(args.seed)
        rng.shuffle(test_samples)
        if val_from_train_ratio:
            split_seed = f"{args.seed}:{fold_name}:val"
            train_samples, val_samples = _split_train_val(train_pool, args.val_ratio, split_seed)
        else:
        ...
        if not args.local_only:
            ...
            if not args.skip_preprocess:
                ...
                logging.info(
                    "Fold %s - Train sessions: %s | Val: %s | Test session: %s",
                    fold_name,
                    train_sessions,
                    f"{args.val_ratio:.3f} of train pool" if val_from_train_ratio else f"session {val_session}",
                    test_session,
                )
                logging.info(
            ...
            if args.skip_features:
                ...
                for client_id in client_ids:
                    ...
                    if val_from_train_ratio:
                        client_train_samples, client_val_samples = _split_train_val(
                            client_samples,
                            args.val_ratio,
                            f"{args.seed}:{fold_name}:{client_name}:val",
                        )
                    else:
                        client_train_samples = client_samples
                        client_val_samples = list(val_samples)
                    client_entries.append({
            ...
            for entry in client_entries:
                ...
                client_val_samples = entry.get("val_samples")
                ...
                if not args.skip_preprocess:
                    ...
                    with open(os.path.join(client_meta_dir, "val.pkl"), "wb") as f:
                        pickle.dump(client_val_samples, f)
    ...
    summary = {
        ...
        "val_ratio": args.val_ratio if args.val_session is None else None,
        "val_source": "train_split" if args.val_session is None else "session",
