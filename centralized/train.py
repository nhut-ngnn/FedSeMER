...
def parse_args():
    parser = argparse.ArgumentParser(description="Train multimodal SER models on IEMOCAP/MSP-IMPROV/ESD/MELD")
    ...
    parser.add_argument("--seeds", type=str, default=None,
                        help="Comma-separated seed list. Defaults to 42,52,103.")
...
def _parse_seeds(value):
    ...
    if isinstance(value, (list, tuple)):
        seeds = [int(item) for item in value if str(item).strip()]
        return seeds or [42, 52, 103]
...
def _checkpoint_path(args, ratio_name_component, model_display_name, seed, stage):
    run_name = _safe_name(args.results_suffix or args.exp_name)
    suffix = "_semi" if stage == "semi" else ""
    filename = (
        f"{run_name}_{args.dataset}_{args.num_classes}class_"
        f"{ratio_name_component}_{model_display_name}_seed{seed}{suffix}.pt"
    )
    return os.path.join("saved_model", filename)


def _remove_stale_checkpoint(path):
...
def main():
    ...
    if lambda_u_value is None:
        if args.pseudo_weight is not None:
            ...
            lambda_u_value = 1.0
    result_suffix = (
        f"{model_display_name}_{len(seeds)}seed"
        if not use_pseudo
        else f"{model_display_name}_SemiPseudo_{len(seeds)}seed"
    )
    ...
    for seed in seeds:
        ...
        save_path_supervised = _checkpoint_path(
            args, ratio_name_component, model_display_name, seed, stage="supervised"
        )
        _remove_stale_checkpoint(save_path_supervised)
        ...
        if use_pseudo and pseudo_available > 0:
            ...
            if unlabeled_dataset is None or len(unlabeled_dataset) == 0:
                ...
                save_path_semi = _checkpoint_path(
                    args, ratio_name_component, model_display_name, seed, stage="semi"
                )
                _remove_stale_checkpoint(save_path_semi)
    ...
    print(f"\n=== Average Results over {len(seeds)} seed(s) ===")
    print(f"Avg WA:  {np.mean(wa_list):.4f}, {_std(wa_list):.4f}")
    print(f"Avg UA:  {np.mean(ua_list):.4f}, {_std(ua_list):.4f}")
    print(f"Avg WF1: {np.mean(wf1_list):.4f}, {_std(wf1_list):.4f}")
    print(f"Avg UF1: {np.mean(uf1_list):.4f}, {_std(uf1_list):.4f}")

    results_df = pd.DataFrame({
        "Metric": ["WA", "UA", "WF1", "UF1"],
        "Mean": [np.mean(wa_list), np.mean(ua_list), np.mean(wf1_list), np.mean(uf1_list)],
        "Std": [_std(wa_list), _std(ua_list), _std(wf1_list), _std(uf1_list)]
    })
