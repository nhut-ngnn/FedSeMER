...
def _collect_msp_improv_loso_samples(data_root, ignore_length):
    ...
    if not os.path.isdir(audio_root):
        parent_root = os.path.dirname(os.path.normpath(data_root))
        parent_audio_root = os.path.join(parent_root, "Audio")
        if os.path.isdir(parent_audio_root):
            logging.warning(
                "MSP-IMPROV data_root=%s does not contain Audio; using parent root %s.",
                data_root,
                parent_root,
            )
            data_root = parent_root
            audio_root = parent_audio_root
            ...
            raise FileNotFoundError(
                f"MSP-IMPROV audio root not found: {audio_root}. "
                "Set data_root to the directory that directly contains Audio/."
            )
